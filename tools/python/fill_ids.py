...
def ids_read(
    backend_in, database_in, shot_no, run_in, user_in, ids_name, occurrence_in
):
    """Method to retrieve data from the database"""
    input = imas.DBEntry(backend_in, database_in, shot_no, run_in, user_in)
    status, _ = input.open()
    if status != 0:
...
def ids_fill(
    ...
    output = imas.DBEntry(
        backend_out, database_out, shot_no, run_out, os.getenv("USER")
    )
    output_folder = os.getenv("HOME") + "/public/imasdb/" + database_out + "/3/0"
