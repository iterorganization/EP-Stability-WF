...
def run_analysis(wf_param_folder, l):
    ...
    if l == 0:
        ...
        result = subprocess.run(
            [
                "general_plots_ids.py",
                f"-user={analysis_param[ana_ref]['user']}",
                f"-database={analysis_param[ana_ref]['database']}",
                f"-shot={analysis_param[ana_ref]['shot_number']}",
                f"-run={analysis_param[ana_ref]['run']}",
                f"-occurrence={occurrence}",
                f"-type=frequency",
                f"-interactivePlots=0",
                f"-compare_modes={analysis_param[ana_ref]['compare_modes']}",
            ],
            capture_output=True,
            text=True,
        )
