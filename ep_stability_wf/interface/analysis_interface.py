...
from ep_stability_wf.interface.create_workflow_param import save_xml_param_to_file, update_xml_param, create_xml_param_from_file
...
def analysis_window(ana_ref, analysis_param, wf_param_folder):
    ...
    # RIGHT SIDE TOP
    button_analysis = Button(fr_ana, text='Frequency, Damping or Radial Location', bg=col.c2)
    button_analysis.grid(row=1, column=3, padx=5, pady=5, sticky='ew')
    button_analysis.configure(command=lambda: save_analysis_open_window(analysis_param, wf_param_folder, label = 'Frequency and Damping'))
    ...
    button_analysis.configure(command=lambda: save_analysis_open_window(analysis_param, wf_param_folder, label = 'Mode Structure'))
...
def window_frequency_damping(wf_param_folder):
    ...
    button_run_damping = Button(
        fr_ana, text='Run Radial Location', bg=col.c2)
