...
def ids_window(ids_ref, wf_param_folder):
    ...
    fr_l_settings.grid(row=0, column=1, rowspan=2, sticky="nwes", padx=3, pady=3)
                ...
                if elem in ["HDF5_1", "HDF5_2", "HDF5_out"]:
                    ...
                    entrystring.trace(
                        ...
                        lambda name, index, mode, elem=elem, entrystring=entrystring, ref=ref: update_xml_param_wf(
                            ids_merge_param, ref, elem, entrystring.get()
                        ),
                    )
                    irow += 1
                else:
    ...
    button_saveconfig = Button(fr_l, text="Save IDS_MERGE Configuration", bg=col.c2)
    ...
    button_saveconfig = Button(fr_l, text="Run IDS_MERGE", bg=col.c2)
