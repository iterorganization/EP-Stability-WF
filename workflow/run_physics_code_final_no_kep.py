...
if param['modus'] == 5:
  ...
  for itime in range(0,time_runs + 1):
      ...
      input.mhd_linear = ligka_actor(input.equilibrium,input.core_profiles,input.mhd_linear,'input/z_ligka.xml','mpi_local')
...
if param['modus'] == 4:
  ...
  for itime in range(0,time_runs + 1):
      ...
      output.mhd_linear = ligka_actor(input.equilibrium,input.core_profiles,input.mhd_linear,'input/z_ligka.xml','mpi_local',mpi_processes=1)
      input.mhd_linear = copy.deepcopy(output.mhd_linear)
      input.mhd_linear.setPulseCtx(idx_in)
...
if param['modus'] == 1:
  ...
  for itime in range(0,time_runs + 1):
      ...
      output.mhd_linear = ligka_actor(input.equilibrium,input.core_profiles,input.mhd_linear,'input/z_ligka.xml','mpi_local',mpi_processes=4)
      input.mhd_linear = copy.deepcopy(output.mhd_linear)
      input.mhd_linear.setPulseCtx(idx_in)
