...
def run_HL_noKEP():
    ...
    if param_ligka['modus'] == 11:
        print('HAGIS 1 EQUILIBRIUM CALCULATION START')
        ...
        for itime in range(0, time_runs + 1):
            ...
            output.equilibrium, output.mhd_linear = hagis1_actor(
                input.equilibrium, input.mhd_linear, 'workflow/input/hagis1.xml')

            input.equilibrium.copyValues(output.equilibrium)
            #input.equilibrium = copy.deepcopy(output.equilibrium)
            input.equilibrium.setPulseCtx(idx_in)
            ...
            print('Saved equilibrium from HAGIS under oc 1')
            print('*************************************')
    ...
    if param_ligka['modus'] == 5:
        ...
        for itime in range(0, time_runs + 1):
            ...
            input.mhd_linear = ligka_actor(
                input.equilibrium, input.core_profiles, input.mhd_linear, 'workflow/input/z_ligka.xml', 'mpi_local')
    ...
    if param_ligka['modus'] == 1:
        ...
        for itime in range(0, time_runs + 1):
            ...
            output.mhd_linear = ligka_actor(input.equilibrium, input.core_profiles,
                                            input.mhd_linear, 'workflow/input/z_ligka.xml', 'mpi_local', mpi_processes=8)
            #input.mhd_linear = copy.deepcopy(output.mhd_linear)
            input.mhd_linear.copyValues(output.mhd_linear)
            input.mhd_linear.setPulseCtx(idx_in)
